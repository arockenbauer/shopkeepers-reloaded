...
class CreateListener implements Listener {
	...
	void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (action == Action.RIGHT_CLICK_AIR || action == Action.LEFT_CLICK_AIR) {
			// Si le joueur a la permission admin et fait clic droit, ouvrir directement l'éditeur d'œuf
			if (action == Action.RIGHT_CLICK_AIR && PermissionUtils.hasPermission(player, ShopkeepersPlugin.ADMIN_PERMISSION)) {
				// Ouvrir l'éditeur d'œuf directement pour les admins
				openEggEditor(player, itemInHand);
				return;
			}
			// Les clics sur l'air sont désactivés - utilisez /shopkeepers eggedit à la place
			TextUtils.sendMessage(player, Messages.useEggEditCommand);
			return;
	...
	/**
	 * Ouvre l'éditeur d'œuf de création pour un admin.
	 */
	private void openEggEditor(Player player, ItemStack shopCreationEgg) {
		// Vérifier que le joueur tient bien un œuf de création
		if (!ShopCreationItem.isShopCreationItem(shopCreationEgg)) {
			TextUtils.sendMessage(player, Messages.mustHoldShopCreationEgg);
			return;
		}

		// Utiliser la commande EggEdit pour ouvrir l'éditeur
		...
		// Ouvrir l'éditeur d'œuf directement (page 0)
		eggEditCommand.openShopCreationEggEditor(player, shopCreationEgg, 0);
	}
}
